...
- [x] `server/utils/eventBus.ts` — per-conversation pub/sub
...
- [x] `submit(msg, source)` with source: `'user' | 'scheduler' | 'cascade'`
- [x] `abort()`, `respondToPermission()`, `resetContext()`, `cleanup()`
- [x] `getJob()`, `getActiveJob()`, `listJobs()`, `listAllJobs()`
...
- [x] Subscribes peers to EventBus for real-time forwarding
...
- [x] `server/api/jobs/[id].get.ts` — GET `/api/jobs/:id`
...
- [x] `server/api/jobs/[id]/cancel.post.ts` — POST `/api/jobs/:id/cancel`
...
- [x] `handleSubscribe()` — subscribes peer to EventBus
- [x] Replays buffered events: `replay_start` → events → `replay_end`
- [x] Returns `subscribed` if no events to replay

### T-010: Add Client Subscribe composable [FR-010]
- [x] `useChatStream.subscribe(conversationId, messageId, cursor?)` function
...
- [x] `refreshServerConversations()` in `stores/chat.ts`
