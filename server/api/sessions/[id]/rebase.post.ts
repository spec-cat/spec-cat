import { rebaseSession } from '../../../utils/session-integration'
...
export default defineEventHandler(async (event) => {
  ...
  try {
    return await rebaseSession(id, body.baseBranch)
  } catch (error) {
    const conflictFiles = getConflictFiles(error)
    throw createError({
      statusCode: conflictFiles.length ? 409 : 400,
      statusMessage: error instanceof Error ? error.message : 'Rebase failed',
      data: { conflictFiles }
    })
  }
})
